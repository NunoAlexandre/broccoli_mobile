...
private class ScrollableGraphViewDrawingLayer : CAShapeLayer {
    ...
    init(viewportWidth: CGFloat, viewportHeight: CGFloat, offset: CGFloat = 0) {
        ...
        self.frame = CGRect(origin: CGPoint(x: offset, y: 0),
                            size: CGSize(width: self.viewportWidth, height: self.viewportHeight))
...
private class BarDrawingLayer: ScrollableGraphViewDrawingLayer {
    ...
    private func createPath (selectedIndex:Int) -> UIBezierPath {
        ...
        guard let activePointsInterval = self.graphViewDrawingDelegate?.intervalForActivePoints() else {
                return barPath
        }
        ...
        for i in activePointsInterval {
            ...
            barPath.append(createBarPath(isSelected: i == selectedIndex, centre: location))
        }
...
private class DataPointDrawingLayer: ScrollableGraphViewDrawingLayer {
    ...
    private func createCircleDataPoint(centre: CGPoint) -> UIBezierPath {
        return UIBezierPath(arcCenter: centre, radius: dataPointSize, startAngle: 0,
                            endAngle: CGFloat(2.0 * .pi), clockwise: true)
    }
